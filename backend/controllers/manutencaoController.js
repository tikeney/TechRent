  ...
  try {
    const [linhas] = await db.query(
      `SELECT
       ...
       JOIN usuarios    t ON t.id = h.tecnico_id
...
// Após registrar, atualiza chamados.status para 'resolvido'
  ...
  try {
    ...
    if (chamado.status === 'resolvido' || chamado.status === 'cancelado') {
      return res.status(400).json({
        mensagem: `Chamado já está ${chamado.status}. Não é possível registrar manutenção.`,
      });
    }

    if (Number(chamado.equipamento_id) !== Number(equipamento_id)) {
      return res.status(400).json({
        mensagem: 'O equipamento_id fornecido não corresponde ao equipamento do chamado.',
      });
    }
    ...
    await db.query(
      "UPDATE chamados SET status = 'resolvido', atualizado_em = NOW() WHERE id = ?",
      [chamado_id]
    );
    ...
    return res.status(201).json({
      ...
      manutencao_id: resultado.insertId,
    });
  } catch (error) {
